...
export function PracticeAreaItemsAdmin({ items: initialItems }: { items: PracticeAreaRow[] }) {
  ...
  function handleMove(id: string, direction: 'up' | 'down') {
    ...
    movePracticeArea(id, direction).then((result) => {
      if (result.error) {
  ...
  function handleTogglePublished(id: string, next: boolean) {
    ...
    setPracticeAreaPublished(id, next).then((result) => {
      if (result.error) {
        setListError(resolveSiteContentError(result.error, tErrors))
        setItems((prev) => prev.map((item) => (item.id === id ? { ...item, is_published: !next } : item)))
      }
...
function AddPracticeAreaForm({ onAdded }: { onAdded: (item: PracticeAreaRow) => void }) {
  const t = useTranslations('dashboard.admin.siteContent')
  const tErrors = useTranslations('dashboard.admin.siteContent.errors')
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await addPracticeArea(formData)
      if (result.error) {
  ...
  return (
    <Panel className="flex flex-col gap-3">
      <h2 className="font-heading text-lg text-fg">{t('items.addPracticeArea')}</h2>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <BilingualField
          ...
          defaultValueEn={nameEn}
          defaultValueAr={nameAr}
        ...
        <BilingualField
          ...
          idSuffix="new"
          labelEn={t('items.descriptionEnLabel')}
          labelAr={t('items.descriptionArLabel')}
          defaultValueEn=""
          defaultValueAr=""
          multiline
          warningLabel={t('statusMissingEnglish')}
...
function PracticeAreaItemPanel({
  item,
  isFirst,
  isLast,
  onMove,
  ...
  item: PracticeAreaRow
  isFirst: boolean
  isLast: boolean
  onMove: (id: string, direction: 'up' | 'down') => void
  onTogglePublished: (id: string, next: boolean) => void
  onDeleted: (id: string) => void
  onUpdated: (id: string, fields: Partial<PracticeAreaRow>) => void
}) {
  const t = useTranslations('dashboard.admin.siteContent')
  const tErrors = useTranslations('dashboard.admin.siteContent.errors')
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const formData = new FormData(e.currentTarget)
    startSave(async () => {
      const result = await updatePracticeArea(item.id, formData)
      if (result.error) {
  ...
  function handleConfirmDelete() {
    setDeleteError(null)
    startDelete(async () => {
      const result = await deletePracticeArea(item.id)
  ...
  return (
    <Panel
      ...
      data-testid={`practice-area-item-${item.id}`}
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-1.5">
          <Button
            ...
            data-testid={`practice-area-item-${item.id}-move-up`}
          >
          ...
          <Button
            ...
            data-testid={`practice-area-item-${item.id}-move-down`}
          >
            <ArrowDownIcon className="h-4 w-4" />
          </Button>
          {!item.is_published && (
            <Badge variant="muted" data-testid={`practice-area-item-${item.id}-hidden-badge`}>
              {t('items.hiddenBadge')}
            </Badge>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            data-testid={`practice-area-item-${item.id}-publish-toggle`}
          />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <BilingualField
          ...
          labelEn={t('items.descriptionEnLabel')}
          labelAr={t('items.descriptionArLabel')}
          defaultValueEn={item.description_en ?? ''}
          defaultValueAr={item.description_ar ?? ''}
          multiline
          warningLabel={t('statusMissingEnglish')}
          warningNote={t('missingEnglishNote')}
        />
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            disabled={isDeleting}
            data-testid={`practice-area-item-${item.id}-delete`}
          >
            {isDeleting ? t('items.deleting') : t('items.delete')}
          </Button>
        </div>
        {saveError && (
          <FieldError data-testid={`practice-area-item-${item.id}-save-error`}>
            <bdi>{saveError}</bdi>
          </FieldError>
        )}
        {deleteError && (
          <FieldError data-testid={`practice-area-item-${item.id}-delete-error`}>
            <bdi>{deleteError}</bdi>
          </FieldError>
        )}
      </form>
